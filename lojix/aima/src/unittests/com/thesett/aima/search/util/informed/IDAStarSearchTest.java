...
/**
 * IDAStarSearchTest is a pure unit test class for the {@link IDAStarSearch} class.
 *
 * @author Rupert Smith
 */
public class IDAStarSearchTest extends TestCase
{
    /**  */
    /* private static final Logger log = Logger.getLogger(IDAStarSearchTest.class.getName()); */

    /** The IDAStarSearch to test. */
    private IDAStarSearch testSearch;

    /** Default constructor that will result in the tests being run on a depth first search. */
    public IDAStarSearchTest(String testName)
    {
        super(testName);

        // Create an iterative cost increasing search to test that goes 1.0f more costly at every iteration.
        // Dummy heuristic alwyas scores 0.0 so the heuristic is equal to the cost.
        testSearch = new IDAStarSearch(new DummyHeuristic(), 0.0f, 1.0f);
    }
    ...
    public static Test suite()
    {
        ...
        TestSuite suite = new TestSuite("IDAStarSearch Tests");
        ...
        // For test traversable states and dummy heuristic the f-value always equals the depth.
        suite.addTest(new BaseQueueSearchTest("testStartNodeEnqueued",
                new IDAStarSearch(new DummyHeuristic(), 0.0f, 1.0f)));
        suite.addTest(new BaseQueueSearchTest("testRepeatedStateFilterReset",
                new IDAStarSearch(new DummyHeuristic(), 0.0f, 1.0f)));
        suite.addTest(new BaseQueueSearchTest("testNoStartStateFails",
                new IDAStarSearch(new DummyHeuristic(), 0.0f, 1.0f)));
        suite.addTest(new BaseQueueSearchTest("testNoGoalSearchReturnsNull",
                new IDAStarSearch(new DummyHeuristic(), 0.0f, 1.0f)));
        suite.addTest(new BaseQueueSearchTest("testSearchReturnsGoal",
                new IDAStarSearch(new DummyHeuristic(), 0.0f, 1.0f)));

        // Run these tests starting at cost 1.0f and incrementing to the next minimal f-value seen beyond the previous
        // bound.
        // For test traversable states and the dummy heuristic the f-value always equals the depth.
        suite.addTest(new BaseQueueSearchTest("testStartNodeEnqueued", new IDAStarSearch(new DummyHeuristic(), 0.0f)));
        suite.addTest(new BaseQueueSearchTest("testRepeatedStateFilterReset",
                new IDAStarSearch(new DummyHeuristic(), 0.0f)));
        suite.addTest(new BaseQueueSearchTest("testNoStartStateFails", new IDAStarSearch(new DummyHeuristic(), 0.0f)));
        suite.addTest(new BaseQueueSearchTest("testNoGoalSearchReturnsNull",
                new IDAStarSearch(new DummyHeuristic(), 0.0f)));
        suite.addTest(new BaseQueueSearchTest("testSearchReturnsGoal", new IDAStarSearch(new DummyHeuristic(), 0.0f)));

        // Add all the tests defined in this class (using the default constructor)
        suite.addTestSuite(IDAStarSearchTest.class);
