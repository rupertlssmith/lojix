 ...
 * <li>Consumers may be able to batch data as they consume it leading to more efficient consumption over individual data
 * item consumption where latency associated with the consume operation that can be ammortized. For example, it may be
 * possibly to ammortize the cost of a disk seek over many producers.</li>
 ...
 *
 * <p/>This type of queue is usefull in situations where consumers can obtain an efficiency gain by batching data from
 * many threads but where synchronous handling of that data is neccessary because producers need to know that their data
 * has been processed before they continue. For example, sending a bundle of messages together, or writing many records
 * to disk at once, may result in improved performance but the originators of the messages or disk records need
 * confirmation that their data has really been sent or saved to disk.
 *
 ...
 * which errors or re-queuings can be applied. If these methods are used then the consumer must succesfully process all
 * the records it takes.
 *
 * <p/>The {@link #put} method should silently swallow any exceptions that consumers attempt to return to the caller. In
 * order to handle exceptions the {@link #tryPut} method must be used.
 *
